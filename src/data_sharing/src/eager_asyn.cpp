...
{
  ros::init(argc, argv, "eager_asyn");

  ros::NodeHandle nh;
  ros::Publisher req_pub, data_pub;
  ...
  while(ros::ok())
  {
    ...
    for(set<int>::iterator it=recv_req.begin(); it!=recv_req.end(); it++) //request和data_avail的交集
    {
      if(data_avail.count(*it))
      {
